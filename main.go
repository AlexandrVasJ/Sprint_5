...
func (t Training) meanSpeed() float64 {
	// вставьте ваш код ниже
	mSpeed := float64(t.Action) * t.LenStep / t.Duration.Hours() / MInKm
	return mSpeed
}
...
func (r Running) Calories() float64 {
	// вставьте ваш код ниже
	speed := r.meanSpeed()
	summ := ((CaloriesMeanSpeedMultiplier*speed + CaloriesMeanSpeedShift) * r.Weight / MInKm * r.Duration.Hours() * MinInHours)
	return summ
}
...
func (w Walking) Calories() float64 {
	...
	summ := ((CaloriesWeightMultiplier*w.Weight + (math.Pow(speed, 2)/height)*CaloriesSpeedHeightMultiplier*w.Weight) * w.Duration.Hours() * MinInHours)
	return summ
}
...
func (s Swimming) meanSpeed() float64 {
	// вставьте ваш код ниже
	mSpeed := float64(s.LengthPool*s.CountPool) / MInKm / s.Duration.Hours()
	return mSpeed
}
...
func (s Swimming) Calories() float64 {
	...
	summ := (s.meanSpeed() + SwimmingCaloriesMeanSpeedShift) * SwimmingCaloriesWeightMultiplier * s.Weight * s.Duration.Hours()
	return summ
}
